...
@Slf4j
@WebServlet("/del-location")
public class DelLocationServlet extends BaseServlet{
    private final UserDao userDao = new UserDao();
    @Override
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        log.info("Deleting location for " + ((Session) req.getAttribute("session")).getUser());
        ...
        log.info("Deleting a " + session.getUser() + " to location");
        userDao.delLocationToUser(session.getUser(), location);
